								...
								@foreach($kelas as $value)
								    ...
								    <td>{{ $value->kompetensi_keahlian }}</td>
											...
											<form method="post" action="{{ url('dashboard/data-kelas', $value->id) }}" id="delete{{ $value->id }}">
												...
												<button type="button" class="dropdown-item" onclick="deleteData({{ $value->id }}, '{{ $value->nama_kelas }}')">
...
@section('sweet')
   ...
   function deleteData(id, nama_kelas){
      Swal.fire({
         title: 'PERINGATAN!',
         text: "Data tidak Dapat Dihapus Karena Memiliki Keterkaitan Dengan Data Lain Di Sistem",
         icon: 'warning'
      })
   }
