...
@section('content')
   ...
   <div class="row">
    ...
    <input
        ...
        name="nis"
        id="nisInput" {{-- Added ID for JavaScript access --}}
        value="{{ old('nis') }}"
        maxlength="8"  {{-- Added for client-side length restriction --}}
        minlength="8"  {{-- Added for client-side length restriction --}}
        onkeyup="validateNIS()" {{-- Calls JavaScript function on key release --}}
    >
    {{-- Original error display (for server-side validation, e.g., Laravel) --}}
    <span class="text-danger @error('nis') d-block @enderror">
        @error('nis') {{ $message }} @enderror
    </span>
    {{-- Custom JavaScript validation error message --}}
...
<script>
    function validateNIS() {
        ...
        if (nisValue.length !== 8) {
            nisError.textContent = 'NIS harus terdiri dari 8 digit';
